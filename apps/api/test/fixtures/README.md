...
3. Creates one **purchasing record (RFQ)** per manufacturer, titled `Elecdes BOM: <manufacturer>`.
4. Creates **purchasing lines** with quantity from `QTY` / `QUANTITY` / …, unit from `UNIT` / `UOM`, description from part/catalog + description fields.

If your export uses different column names, extend the alias lists in `apps/api/src/lib/bomDbfImport.ts`.

## UI

On a project’s **Purchasing / PO** panel, use **Import Elecdes BOM (.dbf)** to upload.
