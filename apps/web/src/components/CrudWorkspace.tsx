...
export function CrudWorkspace({
  ...
  const rowsByTab: Record<Tab, { dataHeaders: string[]; rows: TableRow[] }> = {
    customers: {
      ...
      rows: clients.map((c) => ({
        key: c.id,
        action: rowActions("customers", c.id, c.name, undefined, { openHref: `/workspace/customers/${c.id}` }),
        cells: [
          <InlineText
            key="n"
            value={c.name}
    ...
    suppliers: {
      ...
      rows: suppliers.map((s) => ({
        key: s.id,
        action: rowActions("suppliers", s.id, s.name, undefined, { openHref: `/workspace/suppliers/${s.id}` }),
        cells: [
          <InlineText
            key="n"
            value={s.name}
    ...
    projects: {
      ...
      rows: projects.map((p) => ({
        key: p.id,
        action: rowActions("projects", p.id, p.name, undefined, { openHref: `/workspace/projects/${p.id}` }),
        cells: [
          <InlineText
            key="n"
            value={p.name}
    ...
    milestones: {
      ...
      rows: milestones.map((m) => ({
        key: m.id,
        action: rowActions("milestones", m.id, m.name, undefined, { openHref: `/workspace/milestones/${m.id}` }),
        cells: [
    ...
    timeEntries: {
      ...
      rows: timeEntries.map((te) => ({
        ...
        action: rowActions("timeEntries", te.id, te.note?.trim() || "Time entry", te.userId, {
          openHref: `/workspace/time-entries/${te.id}`,
        }),
        cells: [
